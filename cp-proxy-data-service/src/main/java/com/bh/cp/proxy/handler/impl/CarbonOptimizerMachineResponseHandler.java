...
@Component
public class CarbonOptimizerMachineResponseHandler<T> extends JsonResponseHandler<T> {
	private static final Logger logger = LoggerFactory.getLogger(CarbonOptimizerMachineResponseHandler.class);
	...
	public CarbonOptimizerMachineResponseHandler(HttpServletRequest httpServletRequest, ProxyService proxyService,
			@Value("${carbon.optimizer.recursive.machine.widget-id}") Integer carbonOptimizerWidgetId,
	...
	protected Object parse(Map<String, Object> request) {
		...
		try {
			...
			String machineId = replaceValuesMap.get(ProxyConstants.KEY_ASSET_ID);
			...
			if (jsonArray.length() != 0) {
				...
				for (int i = 0; i < machinesList.size(); i++) {
					outputList.addAll(machinesList.get(i));
				}
				if (level.equals(ProxyConstants.LEVEL_MACHINES)) {
					groupName = retriveGroupName(filteredGroupMachinesList, machineId);
					filteredGroupMachinesList = new ArrayList<>();
					filteredGroupMachinesList.add(createMap1(groupName, machineId));
					Optional<String> optionalMachine = outputList.stream().filter(id -> id.equals(machineId))
							.findFirst();
					machinesPlaceholder = optionalMachine.isPresent() ? optionalMachine.get() : "";

				}
				expectedFilteredResponse = new JSONObject().put(WidgetConstants.DATA, filterExpectedResponse(
						filteredGroupMachinesList, fetchMachinesInfo(request, machinesPlaceholder)));
			}
	...
	@SuppressWarnings("unchecked")
	private String retriveGroupName(List<Map<String, Object>> filteredGroupMachinesList, String machineId) {
		String retrivedGroupName = "";
		for (Map<String, Object> group : filteredGroupMachinesList) {
			List<String> machines = (List<String>) group.get(WidgetConstants.MACHINES);
			if (machines.contains(machineId)) {
				retrivedGroupName = (String) group.get(WidgetConstants.TABNAME);
				break;
			}
		}
		return retrivedGroupName;
	}
	...
	public JSONArray filterExpectedResponse(List<Map<String, Object>> filteredGroupMachinesList,
		...
		for (Map<String, Object> obj : filteredGroupMachinesList) {
			...
			filteredMachinesInfoList
					.add(filterExpectedResponse(machinesList.get(0), filterValuesResponse(valuesApiResponse)));
			filteredGroupMachines
					.put(new JSONObject().put(WidgetConstants.GROUPNAME, groupMachinesMap.get(WidgetConstants.TABNAME))
	...
	public Map<String, Object> createMap1(String title, String value) {
		List<String> machinesList = new ArrayList<>();
		machinesList.add(value);
		Map<String, Object> map = new HashMap<>();
		map.put(WidgetConstants.TABNAME, title);
		map.put(WidgetConstants.MACHINES, machinesList);
		return map;
	}
