...
@Component
public class KPISrResponseHandler<T> extends JsonResponseHandler<T> {
	private static final Logger logger = LoggerFactory.getLogger(KPISrResponseHandler.class);

	@SuppressWarnings("unchecked")
	public KPISrResponseHandler() {
		...
		try {
			...
			if (dataObject.length() != 0) {
				...
				for (int i = 0; i < dataObject.length(); i++) {
					...
					Double value = object.optDouble(WidgetConstants.SR, 0.0);
					outputList.add(createMap(WidgetConstants.SR, formatetoDecimalPlace(value).toString()));
				}
