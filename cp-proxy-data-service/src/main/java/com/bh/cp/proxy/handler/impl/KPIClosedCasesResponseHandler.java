...
@Component
public class KPIClosedCasesResponseHandler<T> extends JsonResponseHandler<T> {

	@SuppressWarnings("unchecked")
	public KPIClosedCasesResponseHandler() {
	...
	protected Object parse(Map<String, Object> request) {
		...
		outputList.add(createMap(WidgetConstants.CLOSED, WidgetConstants.NODATAFOUND));
		try {
			...
			if (resourcesArray.length() != 0) {
				...
				Map<String, Integer> countStatusMap = new HashMap<>();
				for (String str : statusList) {
					countStatusMap.put(str, countStatusMap.getOrDefault(str, 0) + 1);
				}
				outputList = new ArrayList<>();
				outputList.add(
						createMap("Closed Cases", countStatusMap.getOrDefault(WidgetConstants.CLOSEDC, 0).toString()));
