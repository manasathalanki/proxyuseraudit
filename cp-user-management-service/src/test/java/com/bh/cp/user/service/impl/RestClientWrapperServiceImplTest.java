...
class RestClientWrapperServiceImplTest {
	...
	@Test
	void testPutBodyToUrl() {
		when(restTemplate.exchange(anyString(), eq(HttpMethod.PUT), any(HttpEntity.class), eq(String.class)))
				.thenReturn(ResponseEntity.of(Optional.of(responseJson)));
		ResponseEntity<String> actualResponse = restClientWrapperService.putBodyToUrl(mockHttpServletRequest,
				url,requestJson);
		assertEquals(responseJson, actualResponse.getBody());
	}

	@Test
	void testDeleteBodyToUrl() {
		when(restTemplate.exchange(anyString(), eq(HttpMethod.DELETE), any(HttpEntity.class), eq(String.class)))
				.thenReturn(ResponseEntity.of(Optional.of(responseJson)));
		ResponseEntity<String> actualResponse = restClientWrapperService.deleteBodyToUrl(mockHttpServletRequest,
				url,requestJson);
		assertEquals(responseJson, actualResponse.getBody());
	}
