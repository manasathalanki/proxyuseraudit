...
import com.bh.cp.user.constants.JwtUtilConstants;
...
public class JwtUtil {
	...
	private final String adminTokenGrantType; 
	...
	public JwtUtil(@Autowired RestTemplate restTemplate, @Value("${keycloak.client-id}") String clientId,
			...
			@Value("${keycloak.admin.token.generation.grant-type}") String adminTokenGrantType) {
		super();
		...
		this.adminTokenGrantType = adminTokenGrantType;
	}
	...
	@Cacheable(value = "admintoken")
	public String generateAdminToken() {
		logger.info("generateToken=> generate admin token");
		LoginResponseDTO loginResponse = generateToken(tokenGenerationUrl, null,
				null, clientId, clientCredValue, adminTokenGrantType);
