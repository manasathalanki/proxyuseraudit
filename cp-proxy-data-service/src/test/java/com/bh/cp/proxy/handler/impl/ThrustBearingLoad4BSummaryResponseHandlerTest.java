...
class ThrustBearingLoad4BSummaryResponseHandlerTest {

	@Mock
	private ThrustBearingLoadSummaryResponseHandler thrustBearingLoadSummaryResponseHandler;

	@InjectMocks
	private ThrustBearingLoad4BSummaryResponseHandler<?> handler;
	...
	void testParseWithNormal() {
		when(thrustBearingLoadSummaryResponseHandler.parse(mockResponseData, request, ProxyConstants.THRUST_4B)).thenReturn(new JSONObject(mockResponseData));
